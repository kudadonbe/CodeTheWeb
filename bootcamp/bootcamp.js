...
console.log("Account:", accountNumber, "Deposit:", depositAmount, "Active?", isActive);
...
console.log("Account Number:", accountExample.accountNumber, "Name:", accountExample.accountName);
...
const findAccount = accNo => accounts.find(acc => acc.accountNumber === accNo);
...
const deposit = (accNo, amount) => {
  ...
  if (amount <= 0)   { console.log("Deposit amount must be positive!"); return; }
...
const withdraw = (accNo, amount) => {
  const acc = findAccount(accNo);
  if (!acc)                 { console.log("Account not found!");           return; }
  if (amount <= 0)          { console.log("Withdrawal amount must be positive!"); return; }
  if (acc.balance < amount) { console.log("Insufficient funds!");          return; }
...
const printMiniStatement = accNo => {
  ...
  acc.transactions.forEach((t, idx) => {
    console.log(`${idx + 1}. ${t.type} (ID ${t.id}) of ${t.amount} on ${t.date} → Balance: ${t.balanceAfter}`);
  });
  console.log(`=======================\n`);
};
